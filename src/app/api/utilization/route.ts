...
async function ownerPondIds(userId: string): Promise<number[]> {
...
export async function GET(req: NextRequest) {
  ...
  let pondIds: number[];
  if (pondsParam === "all") {
    pondIds = isAdmin ? await allPondIds() : await ownerPondIds(session.user.id);
  } else if (pondsParam === "mine") {
    pondIds = await ownerPondIds(session.user.id);
  } else {
    ...
    if (isAdmin) {
      ...
      const owned = new Set(await ownerPondIds(session.user.id));
      pondIds = requested.filter((id) => owned.has(id));
      if (pondIds.length === 0) {
  ...
  if (pondIds.length === 0) {
    return NextResponse.json({
      data: [],
      message: "No ponds accessible for the current user.",
    });
  }
