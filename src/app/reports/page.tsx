...
type ScopeMode = 'all' | 'mine' | 'select';
	...
	return (
		<MainLayout>
			<div className="space-y-8">
				...
				<ExportAllSection isAdmin={isAdmin} ponds={ponds} />
...
function ExportAllSection({
	...
	const [scopeMode, setScopeMode] = useState<ScopeMode>(isAdmin ? 'all' : 'mine');
	...
	useEffect(() => {
		if (!isAdmin && scopeMode === 'all') setScopeMode('mine');
	}, [isAdmin, scopeMode]);
	...
	function resolveScope(): AllReadingsScope | null {
		...
		if (scopeMode === 'mine') return { mode: 'mine' };
			...
			<div>
				...
				<div className="flex flex-wrap gap-2">
					...
					<button
						...
						className={`${pillBase} ${scopeMode === 'mine' ? pillActive : pillIdle}`}>
						All My Ponds
					</button>
					<button
						type="button"
