...
async function sendOnce(pondId: number, startedAt: number) {
  ...
  try {
    ...
    console.log(
      `[${timeStamp()}] ${tag} | rtd=${data.rtd} ph=${data.ph} sal=${data.sal} dox=${data.dox} -> ${ok ? "" : "ERR "}${res.status}  (next in 15m)`
    );
    if (!ok) {
...
async function pondLoop(pondId: number) {
  const bootDelayMs = Math.floor(Math.random() * 60 * 1000);
  const startedAt = Date.now();

  console.log(
    `[pond-${pondId}] BOOTED. Interval: 15m. First send in ${(bootDelayMs / 1000).toFixed(1)}s.`
  );
